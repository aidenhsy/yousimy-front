...
import { useInsertBrandsEmployeesOne } from '@/hooks/brands-employees.hooks';

interface AddBrandManagerProps {
  open: boolean;
  brandId: any;
  refetch: () => void;
}

const AddBrandManagerModal: React.FC<AddBrandManagerProps> = ({
  brandId,
  open,
  refetch,
}) => {
  const closeModal = useModalStore((state) => state.closeModal);
  const { insertBrandsEmployeesOne } = useInsertBrandsEmployeesOne();
  ...
  const handleOk = async () => {
    for (let employee of employeeSelection) {
      await insertBrandsEmployeesOne(brandId, employee.id);
    }
    refetch();
...
export default AddBrandManagerModal;
