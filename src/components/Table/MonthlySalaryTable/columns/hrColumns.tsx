...
const hrColumns = (month: string): ColumnsType<any> => {
  return [
    ...
    {
      title: "考勤部门",
      dataIndex: ["attendance_department", "name"],
      key: "attendance_department_name",
      fixed: "left",
    ...
    },
    {
      ...
      render: (val: boolean, record: any) => {
        if (val) {
          return <span className="tw-text-green-500">已确认</span>;
        } else if (!val && record?.refuse_reason) {
          return <span>已拒绝 理由为:{record.refuse_reason}</span>;
        } else {
          return <span className="tw-text-red-500">未确认</span>;
        }
      },
    },
  ];
